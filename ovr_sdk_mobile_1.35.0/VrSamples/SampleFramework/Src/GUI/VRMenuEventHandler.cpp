...
void VRMenuEventHandler::Frame(
    ...
    */
    ...
    /*
        ...
        {
            VRMenuEvent event( VRMENU_EVENT_SWIPE_UP, EVENT_DISPATCH_FOCUS, FocusedHandle, Vector3f(
       0.0f ), result, "" ); events.push_back( event );
        }
        if ( ( vrFrame.AllButtons & BUTTON_SWIPE_DOWN ) != 0 )
        {
            VRMenuEvent event( VRMENU_EVENT_SWIPE_DOWN, EVENT_DISPATCH_FOCUS, FocusedHandle,
       Vector3f( 0.0f ), result, "" ); events.push_back( event );

        }
        if ( ( vrFrame.AllButtons & BUTTON_SWIPE_FORWARD ) != 0 )
        {
            VRMenuEvent event( VRMENU_EVENT_SWIPE_FORWARD, EVENT_DISPATCH_FOCUS, FocusedHandle,
       Vector3f( 0.0f ), result, "" ); events.push_back( event );

        }
        if ( ( vrFrame.AllButtons & BUTTON_SWIPE_BACK ) != 0 )
        {
            VRMenuEvent event( VRMENU_EVENT_SWIPE_BACK, EVENT_DISPATCH_FOCUS, FocusedHandle,
       Vector3f( 0.0f ), result, "" ); events.push_back( event );
        }
    */

    const bool isAButtonDown = (vrFrame.AllButtons & ovrButton_A) != 0;
    const bool wasAButtonDown = (vrFrame.LastFrameAllButtons & ovrButton_A) != 0;

    bool touchPressed = isAButtonDown && !wasAButtonDown;
    bool touchReleased = !isAButtonDown && wasAButtonDown;
    bool touchDown = (vrFrame.AllButtons) != 0;
    bool ignoreTouchRelease =
        false; /// ( vrFrame.Input.buttonState & BUTTON_TOUCH_WAS_SWIPE ) != 0;
