...
ovrVector4f BatteryColors[] = {{0.745f, 0.114f, 0.176f, 1.0f}, {0.92f, 0.361f, 0.176f, 1.0f},
                               {0.976f, 0.69f, 0.255f, 1.0f}, {0.545f, 0.769f, 0.247f, 1.0f},
                               {0.545f, 0.769f, 0.247f, 1.0f}, {0.0f, 0.78f, 0.078f, 1.0f},
...
uint buttonState, lastButtonState;
...
Menu romSelectionMenu, mainMenu, settingsMenu, buttonMapMenu, bottomBar,
...
jlong Java_com_nintendont_virtualboy_MainActivity_nativeSetAppInterface(JNIEnv *jni, jclass clazz,
                                                                jobject activity,
...
void CreateScreen() {
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, MENU_WIDTH, MENU_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
void DrawGUI() {
  ...
  DrawHelper::DrawTexture(textureWhiteId, 0, MENU_HEIGHT - BOTTOM_HEIGHT, MENU_WIDTH, BOTTOM_HEIGHT,
  ...
  float colorState =
      ((batteryLevel * 10) % (1000 / (batteryColorCount))) / (float) (1000 / batteryColorCount);
...
static inline ovrLayerProjection2 vbLayerProjection() {
  ...
  const ovrMatrix4f
      projectionMatrix = ovrMatrix4f_CreateProjectionFov(90.0f, 90.0f, 0.0f, 0.0f, 0.1f, 0.0f);
  const ovrMatrix4f
...
ovrFrameResult OvrApp::Frame(const ovrFrameInput &vrFrame) {
  ...
  LOG("Button State: %i %f, %f", vrFrame.Input.buttonState, vrFrame.Input.sticks[0][0], vrFrame.Input.sticks[1][0]);
...
void OnClickChangeButtonMappingEnter(MenuItem *item) {
  ...
  possibleMappingIndices = BUTTON_TOUCH - 1;// 4194303;// BUTTON_A | BUTTON_B | BUTTON_X | BUTTON_Y;
...
void UpdateButtonMapping(MenuItem *item, uint &_buttonState, uint &_lastButtonState) {
  if (UpdateMapping) {
    ...
    if (false){//newButtonState & possibleMappingIndices) {
      int buttonIndex = 0;
...
void OvrApp::SetUpMenu() {
  ...
  // main menu page
  {
    ...
    mainMenu.MenuItems.push_back(new MenuButton(&fontMenu, textureResetIconId, "Reset Game", posX,
                                                ...
                                                nullptr));
    slotButton =
        new MenuButton(&fontMenu, textureSaveSlotIconId, "", posX, posY += menuItemSize + 10,
  ...
  // settings page
  {
    ...
    settingsMenu.MenuItems.push_back(new MenuLabel(&fontVersion,
                                                   STR_VERSION,
                                                   MENU_WIDTH - 70,
                                                   MENU_HEIGHT - BOTTOM_HEIGHT - 50 + 10, 70, 50,
                                                   textColorVersion));
  ...
  // button mapping
  {
    ...
    for (int i = 0; i < EMULATOR::buttonCount; ++i)
      buttonMapMenu.MenuItems.push_back(buttonMapping[i]);
