...
namespace generate_mipmaps
{
	...
	int test(gli::format Format, genType const & Black, genType const & Color, std::size_t Size, gli::filter Filter)
	{
		...
		gli::texture3d Texture(Format, gli::texture3d::extent_type(static_cast<gli::texture3d::extent_type::value_type>(Size)));
		Texture.clear(Black);
		Texture[0].clear(Color);

		genType const LoadC = Texture.load<genType>(gli::texture3d::extent_type(0), Texture.max_level());
		if(Texture.levels() > 1)
			Error += LoadC == Black ? 0 : 1;

		gli::texture3d TextureView(gli::view(Texture, 0, 0));
		gli::fsampler3D SamplerA(gli::texture3d(gli::duplicate(Texture)), gli::WRAP_CLAMP_TO_EDGE);
		SamplerA.generate_mipmaps(gli::FILTER_LINEAR);

		gli::texture3d MipmapsA = SamplerA();
		genType const LoadA = MipmapsA.load<genType>(gli::texture3d::extent_type(0), MipmapsA.max_level());
		...
		gli::texture3d MipmapsB = gli::generate_mipmaps(gli::texture3d(gli::duplicate(Texture)), Filter);
		genType const LoadB = MipmapsB.load<genType>(gli::texture3d::extent_type(0), MipmapsB.max_level());
