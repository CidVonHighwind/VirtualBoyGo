...
void UiIconSet::Draw(UiRenderer &ui, UiIconId id, float x, float y, float size, float alpha) const
{
    if (id == UiIconId::None)
        return;
    const UiIconUv &uv = kIconUvs[static_cast<size_t>(id)];
    ui.DrawImageRegion(m_atlas, x, y, size, size, uv.u0, uv.v0, uv.u1, uv.v1, alpha);
}
