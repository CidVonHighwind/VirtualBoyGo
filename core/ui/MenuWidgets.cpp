...
void MenuLabel::Draw(UiRenderer &ui, float offsetX, float offsetY, float alpha)
{
    ...
    XrColor4f c = Color;
    c.a *= alpha;
    ui.DrawText(m_font, m_text, PosX + offsetX + (Selected ? 5 : 0), PosY + offsetY, 1.0f, c);
}
...
void MenuButton::Draw(UiRenderer &ui, float offsetX, float offsetY, float alpha)
{
    ...
    c.a *= alpha;
    ui.DrawText(m_font, Text, PosX + (Selected ? 5 : 0) + offsetX + m_offsetX, PosY + offsetY, 1.0f, c);
}
    ...
    : m_font(font), m_posX(posX), m_posY(posY), m_width(width), m_height(height)
{
    ...
    m_textRowOffset = static_cast<int>(m_itemHeight / 2.0f - ui.GetFontPHeight(font) / 2.0f - ui.GetFontPStart(font));
    Selectable = true;
}
...
void MenuList::Draw(UiRenderer &ui, float offsetX, float offsetY, float alpha)
{
    ...
    const int contentWidth = needsScrollbar() ? m_width - kScrollbarWidth - kScrollbarGap : m_width;
    ...
    const int usedHeight = visible * m_itemHeight;
    const int verticalPad = (m_height - usedHeight) / 2;
    ...
    if (needsScrollbar())
    {
        const int trackX = m_posX + m_width - kScrollbarWidth;
        const int totalEntries = (int)m_entries.size();
        const int trackPad = 3; // px gap at top/bottom so thumb never overflows
        ...
        const float thumbH = std::max(8.0f, trackH * visible / totalEntries);
        ...
        XrColor4f shadow = {0.0f, 0.0f, 0.0f, 0.45f * alpha};
        ui.DrawQuadRounded(trackX + offsetX + 1, thumbY + 1, kScrollbarWidth, thumbH, shadow, radius);
